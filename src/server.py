...
async def radio(
    ...
    """RADIO / NETEASE MUSIC TOOL — use for 网易云音乐、Radio、歌单、找歌或音乐推荐。playlists lists only human playlists explicitly shared in Garden plus Senn-created playlists. playlist reads tracks using original_id and encrypted_id returned by playlists. search finds song/album/playlist/all and remembers each song's originalId → 32-character encryptedId pair; when a result has both originalId and id, id is the encrypted API ID. create_playlist creates a Senn-owned playlist. For add_tracks, first call search for the songs, then pass their encrypted API IDs in songIdList; cached clients may use song_ids with one ID, a list, JSON text, or comma-separated text. Garden also resolves original numeric IDs learned by a preceding search. Both writes require confirm=true. comment leaves Senn's reason under a playlist or track using target_type=playlist/track and note. It never plays audio or reads Memos, journals, or chats."""
