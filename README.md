...
**电台(Radio)** 是没有扬声器的无线电室。Garden 通过网易云音乐官方 `@music163/ncm-cli` 陈列“我创建的”和“Senn 创建的”歌单；你可以逐张选择哪些个人歌单向 `radio` Connector 开放。Senn 可以搜索音乐、在明确确认后创建自己的歌单和加入曲目，也可以在歌单或歌曲下留下选择理由。开放平台 App ID 与 Private Key 在 Radio 页面交给官方 CLI 保存，扫码登录态留在私人数据卷；音乐资料和留言都不会进入 Memos。
